# Design System — mz-finance (base Binance)

> Fornecido pelo dev em 2026-07-02 como o Design System oficial do projeto (substitui o placeholder de CSS Modules previsto no context.md § Frontend).
> Inspiração adicional de UX/layout: https://visorfinance.app/
>
> **Nota de adaptação:** este DS foi extraído da Binance (plataforma de trading). O mz-finance é um app de finanças pessoais — os tokens (cores, tipografia, espaçamento, radius, elevação) aplicam-se diretamente; os componentes de marketing (hero bands, QR promo, arena) servem como referência de linguagem visual, não como telas a construir. Fontes substitutas: **Inter** (no lugar de BinanceNova) e **IBM Plex Sans/Mono** (no lugar de BinancePlex para números).
