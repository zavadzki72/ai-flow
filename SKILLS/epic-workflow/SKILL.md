...
compatibility: Requer subagents em janela isolada (tool `Agent` + `.claude/agents/`), aninhamento até profundidade 5 e paralelismo real (N chamadas numa mesma mensagem) — hoje só Claude Code. Sem isso, use /spec → /planejar → /implementar → /code-review manualmente.
---
...
- **Perguntas** — não existem aqui. Esta skill é **sempre autônoma** (ver § Interação humana).
...
- Um agente só delega se **`Agent` estiver na lista `tools:` dele**: lista explícita sem `Agent` faz
  o agente perder a delegação — o campo omitido herda tudo, inclusive `Agent`. Se o
  `engineering-manager` não tiver `Agent`, ele não dispara os `dev-senior` e a FASE 3 morre em
  silêncio (verificado no Passo 0.4).
...
- **Nunca** adicione trailer `Co-Authored-By` de IA (Claude, Copilot, Cursor, Gemini…) nos commits —
  nem você, nem os agentes que você dispara. Se o seu harness anexa esse trailer por padrão — **o
  Claude Code anexa** — remova-o. Sem exceção.
...
- Se rodou `--so-planejar`: revise os PRDs/PLANs e então `/epic-workflow {path-do-épico}` (épico
  full) ou `/implementar` etapa a etapa.
- Uma feature só, que cabe num PRD: `/feature-workflow` — esta skill não se paga abaixo de ~2
  features que não colidam entre si (SHARED § Quando o épico não se paga).
- Após o épico: push e PR são **sugeridos, nunca executados** — quem decide é o dev.
